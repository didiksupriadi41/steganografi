...
class ImageSteganography:
    ...
    def hide_message(input_choice, input_image, input_message, input_key="", output_image="output.png", pixelSequential=True, from_file=False, encrypt=False):
        ...
        if pixelSequential:
            input_message_new = "1" + input_message
        else:
            input_message_new = "2" + input_message
        ...
        if input_choice[0] == '2':
            ...
            image = ImageSteganography.encodeLSB(image, input_message_new, input_key=input_key, output_image=output_image, pixelSequential=pixelSequential)
    ...
    def embedBitsToPixels(binaryNTuple, pixels):
        ...
        for pixel in pixels:
            ...
            for p in pixel:    
                ...
                if test:
                    test = False
                    print("len(p) ", len(binaryPixel))
                binaryPixelList.append(binaryPixel)
            binaryAllPixelsList.append(binaryPixelList)
        print("len(binaryNTuple) ", len(binaryNTuple))
        print("len(binaryNTuple[i]) ", len(binaryNTuple[0]))
        print("len(binaryAllPixelsList) ", len(binaryAllPixelsList))
        print("binaryAllPixelsList[i] ", len(binaryAllPixelsList[0]))
    ...
    def encodeLSB(input_image, input_message, input_key="", steganographyMethod="", output_image="output.png", pixelSequential=True):
        ...
        binaryNTupleList = ImageSteganography.createBinaryNTupleList(input_message, bit_depth, input_key)
        #binaryTriplePairs = createBinaryTriplePairs(message)
        #print("binaryTriplePairs = ", binaryNTupleList)
        print("len(input_message) = ", len(input_message))
        print("length binaryNTupleList = ", len(binaryNTupleList))


        pixels = list(input_image.getdata())
        #print("pixels = ", pixels)
            
        newPixels = ImageSteganography.embedBitsToPixels(binaryNTupleList, pixels)
